...
class ScalarMixWithDropout(torch.nn.Module):
    ...
    def __repr__(self):
        layer_scalars = [param.data.item() for param in self.scalar_parameters]
        gamma = self.gamma.data.item()

        representation = "("
        for i in range(len(layer_scalars)):
            representation += "\n\tLayer {} scalar: {:.4}".format(i, layer_scalars[i])
